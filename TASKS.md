...
# Real-time Audio Infrastructure [NOT DONE]
- [ ] Install real-time audio libraries (`pyaudio`, `sounddevice`, `queue`)
- [ ] Create `src/demo/audio_stream.py` for microphone capture
- [ ] Implement circular buffer for 30-second audio segments (Whisper requirement)
- [ ] Implement audio chunk accumulation with overlap handling
- [ ] Test microphone input quality and verify 16kHz sampling rate
- [ ] Add audio volume normalization for consistent input levels

# Live Transcription UI System [NOT DONE]
- [ ] Install UI framework (`gradio` recommended for rapid prototyping)
- [ ] Create `src/demo/live_transcribe.py` for main demo application
- [ ] Implement clean transcription mode (baseline Whisper)
- [ ] Add real-time transcript display with auto-scrolling
- [ ] Implement mode selector: [Clean / Untargeted UAP / Targeted Attack]
- [ ] Add visual indicators for attack mode status
- [ ] Display current SNR and WER metrics in real-time
