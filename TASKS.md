...
---
**Progress Log:**
- Implemented real-time audio infrastructure using `sounddevice` and `numpy`.
- Created `src/demo/audio_stream.py` with a circular buffer system capable of handling Whisper's 30-second audio segments at 16kHz.
- Added normalization logic to ensure input audio stays within [-1.0, 1.0].
- Updated `requirements.txt` to include `sounddevice` and `gradio` (UI dependencies).
